...
import com.devidea.business.MotivoSolicitacaoBusiness;
...
@Path("/motivoSolicitacao")
@Produces(MediaType.APPLICATION_JSON)
@Stateless
public class MotivoSolicitacaoResource {

	@Context
	private HttpServletRequest httpServletResquest;
	
	@EJB
	private MotivoSolicitacaoBusiness motivoSolicitacaoBusiness;
...
// VARIABLES_RESOURCE
	...
	public MotivoSolicitacaoResource() {
	...
	public Response getById(Integer id, @Context HttpHeaders headers){
		...
		MotivoSolicitacao entidade = motivoSolicitacaoBusiness.findById(id);
	...
	public Response getAll(@Context HttpHeaders headers){
		...
		List<MotivoSolicitacao> list = motivoSolicitacaoBusiness.findAll();
	...
	public Response getAllFast(@Context HttpHeaders headers){
		...
		List<MotivoSolicitacao> list = motivoSolicitacaoBusiness.findAllFast();
	...
	public Response getAll(Integer page, @Context HttpHeaders headers){
		...
		List<MotivoSolicitacao> list = motivoSolicitacaoBusiness.findAll(page);
	...
	public Response getByExample(String json, @Context HttpHeaders headers){
		...
		MotivoSolicitacao example = gson.fromJson(json, MotivoSolicitacao.class);
		List<MotivoSolicitacao> list = motivoSolicitacaoBusiness.findByExample(example,example.getListPage(),example.getListPageSize());
	...
	public Response create(String json, @Context HttpHeaders headers){	
		...
		MotivoSolicitacao entidade = gson.fromJson(json, MotivoSolicitacao.class);
		entidade.setDataCadastro(new Date());
		entidade.setUsuarioCadastro(usuario);
		
		try {
			entidade = motivoSolicitacaoBusiness.create(entidade);			
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response update(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = motivoSolicitacaoBusiness.update(entidade);
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response listagem(String json, @Context HttpHeaders headers){
		...
		MotivoSolicitacao example = gson.fromJson(json, MotivoSolicitacao.class);
		List<MotivoSolicitacao> list = motivoSolicitacaoBusiness.findByExample(example);

		byte[] bytesReport = null;
		try {
			bytesReport = motivoSolicitacaoBusiness.listagemPDF(this.httpServletResquest, usuario, list);
		...
		return Response.ok(bytesReport).header("Access-Control-Allow-Origin", frontEndResponse)
				...
				.header("Content-Disposition", "filename=\"" + "motivoSolicitacaoList" + ".pdf\"").build();
	}
