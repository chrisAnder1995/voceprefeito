...
import com.devidea.business.StatusBusiness;
...
// VARIABLES_RESOURCE
	...
	public Response getById(Integer id, @Context HttpHeaders headers){
		...
		Status entidade = statusBusiness.findById(id);
	...
	public Response getAll(@Context HttpHeaders headers){
		...
		List<Status> list = statusBusiness.findAll();
	...
	public Response getAllFast(@Context HttpHeaders headers){
		...
		List<Status> list = statusBusiness.findAllFast();
	...
	public Response getAll(Integer page, @Context HttpHeaders headers){
		...
		List<Status> list = statusBusiness.findAll(page);
	...
	public Response getByExample(String json, @Context HttpHeaders headers){
		...
		Status example = gson.fromJson(json, Status.class);
		List<Status> list = statusBusiness.findByExample(example,example.getListPage(),example.getListPageSize());
	...
	public Response create(String json, @Context HttpHeaders headers){	
		...
		Status entidade = gson.fromJson(json, Status.class);
		entidade.setDataCadastro(new Date());
		entidade.setUsuarioCadastro(usuario);
		
		try {
			entidade = statusBusiness.create(entidade);			
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response update(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = statusBusiness.update(entidade);
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response listagem(String json, @Context HttpHeaders headers){
		...
		Status example = gson.fromJson(json, Status.class);
		List<Status> list = statusBusiness.findByExample(example);

		byte[] bytesReport = null;
		try {
			bytesReport = statusBusiness.listagemPDF(this.httpServletResquest, usuario, list);
		...
		return Response.ok(bytesReport).header("Access-Control-Allow-Origin", frontEndResponse)
				...
				.header("Content-Disposition", "filename=\"" + "statusList" + ".pdf\"").build();
	}
