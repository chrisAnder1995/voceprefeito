...
// VARIABLES_RESOURCE
	...
	public Response getById(Integer id, @Context HttpHeaders headers){
		...
		Regiao entidade = religiaoBusiness.findById(id);
	...
	public Response getAll(@Context HttpHeaders headers){
		...
		List<Regiao> list = religiaoBusiness.findAll();
	...
	public Response getAllFast(@Context HttpHeaders headers){
		List<Regiao> list = religiaoBusiness.findAllFast();
	...
	public Response getAll(Integer page, @Context HttpHeaders headers){
		...
		List<Regiao> list = religiaoBusiness.findAll(page);
	...
	public Response getByExample(String json, @Context HttpHeaders headers){
		...
		Regiao example = gson.fromJson(json, Regiao.class);
		List<Regiao> list = religiaoBusiness.findByExample(example,example.getListPage(),example.getListPageSize());
	...
	public Response create(String json, @Context HttpHeaders headers){	
		...
		Regiao entidade = gson.fromJson(json, Regiao.class);
		entidade.setDataCadastro(new Date());
		entidade.setUsuarioCadastro(usuario);
		
		try {
			entidade = religiaoBusiness.create(entidade);			
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response update(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = religiaoBusiness.update(entidade);
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response listagem(String json, @Context HttpHeaders headers){
		...
		Regiao example = gson.fromJson(json, Regiao.class);
		List<Regiao> list = religiaoBusiness.findByExample(example);

		byte[] bytesReport = null;
		try {
			bytesReport = religiaoBusiness.listagemPDF(this.httpServletResquest, usuario, list);
		...
		return Response.ok(bytesReport).header("Access-Control-Allow-Origin", frontEndResponse)
				...
				.header("Content-Disposition", "filename=\"" + "religiaoList" + ".pdf\"").build();
	}
