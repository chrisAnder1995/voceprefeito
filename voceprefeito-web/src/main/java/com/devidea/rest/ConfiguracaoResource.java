...
import com.devidea.business.ConfiguracaoBusiness;
...
// VARIABLES_RESOURCE
	...
	public Response getById(Integer id, @Context HttpHeaders headers){
		...
		Configuracao entidade = configuracaoBusiness.findById(id);
	...
	public Response getAll(@Context HttpHeaders headers){
		...
		List<Configuracao> list = configuracaoBusiness.findAll();
	...
	public Response getAllFast(@Context HttpHeaders headers){
		...
		List<Configuracao> list = configuracaoBusiness.findAllFast();
	...
	public Response getAll(Integer page, @Context HttpHeaders headers){
		...
		List<Configuracao> list = configuracaoBusiness.findAll(page);
	...
	public Response getByExample(String json, @Context HttpHeaders headers){
		...
		Configuracao example = gson.fromJson(json, Configuracao.class);
		List<Configuracao> list = configuracaoBusiness.findByExample(example,example.getListPage(),example.getListPageSize());
	...
	public Response create(String json, @Context HttpHeaders headers){	
		...
		Configuracao entidade = gson.fromJson(json, Configuracao.class);
		entidade.setDataCadastro(new Date());
		entidade.setUsuarioCadastro(usuario);
		
		try {
			entidade = configuracaoBusiness.create(entidade);			
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response update(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = configuracaoBusiness.update(entidade);
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response listagem(String json, @Context HttpHeaders headers){
		...
		Configuracao example = gson.fromJson(json, Configuracao.class);
		List<Configuracao> list = configuracaoBusiness.findByExample(example);

		byte[] bytesReport = null;
		try {
			bytesReport = configuracaoBusiness.listagemPDF(this.httpServletResquest, usuario, list);
		...
		return Response.ok(bytesReport).header("Access-Control-Allow-Origin", frontEndResponse)
				...
				.header("Content-Disposition", "filename=\"" + "configuracaoList" + ".pdf\"").build();
	}
