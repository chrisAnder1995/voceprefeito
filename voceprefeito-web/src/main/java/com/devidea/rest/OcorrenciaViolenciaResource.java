...
@Path("/ocorrenciaViolencia")
@Produces(MediaType.APPLICATION_JSON)
@Stateless
public class OcorrenciaViolenciaResource {
...
// VARIABLES_RESOURCE
	...
	public OcorrenciaViolenciaResource() {
	...
	public Response getById(Integer id, @Context HttpHeaders headers){
		...
		OcorrenciaViolencia entidade = ocorrenciaViolenciaBusiness.findById(id);
	...
	public Response getAll(@Context HttpHeaders headers){
		...
		List<OcorrenciaViolencia> list = ocorrenciaViolenciaBusiness.findAll();
	...
	public Response getAllFast(@Context HttpHeaders headers){
		...
		List<OcorrenciaViolencia> list = ocorrenciaViolenciaBusiness.findAllFast();
	...
	public Response getAll(Integer page, @Context HttpHeaders headers){
		...
		List<OcorrenciaViolencia> list = ocorrenciaViolenciaBusiness.findAll(page);
	...
	public Response getByExample(String json, @Context HttpHeaders headers){
		...
		OcorrenciaViolencia example = gson.fromJson(json, OcorrenciaViolencia.class);
		List<OcorrenciaViolencia> list = ocorrenciaViolenciaBusiness.findByExample(example,example.getListPage(),example.getListPageSize());
	...
	public Response create(String json, @Context HttpHeaders headers){	
		...
		OcorrenciaViolencia entidade = gson.fromJson(json, OcorrenciaViolencia.class);
		entidade.setDataCadastro(new Date());
		entidade.setUsuarioCadastro(usuario);
		
		try {
			entidade = ocorrenciaViolenciaBusiness.create(entidade);			
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response update(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = ocorrenciaViolenciaBusiness.update(entidade);
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response listagem(String json, @Context HttpHeaders headers){
		...
		OcorrenciaViolencia example = gson.fromJson(json, OcorrenciaViolencia.class);
		List<OcorrenciaViolencia> list = ocorrenciaViolenciaBusiness.findByExample(example);

		byte[] bytesReport = null;
		try {
			bytesReport = ocorrenciaViolenciaBusiness.listagemPDF(this.httpServletResquest, usuario, list);
		...
		return Response.ok(bytesReport).header("Access-Control-Allow-Origin", frontEndResponse)
				...
				.header("Content-Disposition", "filename=\"" + "ocorrenciaViolenciaList" + ".pdf\"").build();
	}
