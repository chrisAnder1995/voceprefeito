...
@Stateless
public class MulherDrogaResource {
...
// VARIABLES_RESOURCE
	...
	public Response getById(Integer id, @Context HttpHeaders headers){
		...
		MulherDroga entidade = mulherDrogaBusiness.findById(id);
	...
	public Response getAll(@Context HttpHeaders headers){
		...
		List<MulherDroga> list = mulherDrogaBusiness.findAll();
	...
	public Response getAllFast(@Context HttpHeaders headers){
		...
		List<MulherDroga> list = mulherDrogaBusiness.findAllFast();
	...
	public Response getAll(Integer page, @Context HttpHeaders headers){
		...
		List<MulherDroga> list = mulherDrogaBusiness.findAll(page);
	...
	public Response getByExample(String json, @Context HttpHeaders headers){
		...
		MulherDroga example = gson.fromJson(json, MulherDroga.class);
		List<MulherDroga> list = mulherDrogaBusiness.findByExample(example,example.getListPage(),example.getListPageSize());
	...
	public Response create(String json, @Context HttpHeaders headers){	
		...
		MulherDroga entidade = gson.fromJson(json, MulherDroga.class);
		entidade.setDataCadastro(new Date());
		entidade.setUsuarioCadastro(usuario);
		
		try {
			entidade = mulherDrogaBusiness.create(entidade);			
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response update(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = mulherDrogaBusiness.update(entidade);
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response listagem(String json, @Context HttpHeaders headers){
		...
		MulherDroga example = gson.fromJson(json, MulherDroga.class);
		List<MulherDroga> list = mulherDrogaBusiness.findByExample(example);

		byte[] bytesReport = null;
		try {
			bytesReport = mulherDrogaBusiness.listagemPDF(this.httpServletResquest, usuario, list);
		...
		return Response.ok(bytesReport).header("Access-Control-Allow-Origin", frontEndResponse)
				...
				.header("Content-Disposition", "filename=\"" + "mulherDrogaList" + ".pdf\"").build();
	}
