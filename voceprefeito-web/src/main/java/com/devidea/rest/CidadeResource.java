...
// VARIABLES_RESOURCE
	...
	public Response getById(Integer id, @Context HttpHeaders headers){
		...
		Cidade entidade = cidadeBusiness.findById(id);
	...
	public Response getAll(@Context HttpHeaders headers){
		...
		List<Cidade> list = cidadeBusiness.findAll();
	...
	public Response getAllFast(@Context HttpHeaders headers){
		...
		List<Cidade> list = cidadeBusiness.findAllFast();
	...
	public Response getAll(Integer page, @Context HttpHeaders headers){
		...
		List<Cidade> list = cidadeBusiness.findAll(page);
	...
	public Response getByExample(String json, @Context HttpHeaders headers){
		...
		Cidade example = gson.fromJson(json, Cidade.class);
		List<Cidade> list = cidadeBusiness.findByExample(example,example.getListPage(),example.getListPageSize());
	...
	public Response create(String json, @Context HttpHeaders headers){	
		...
		Cidade entidade = gson.fromJson(json, Cidade.class);
		entidade.setDataCadastro(new Date());
		entidade.setUsuarioCadastro(usuario);
		
		try {
			entidade = cidadeBusiness.create(entidade);			
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response update(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = cidadeBusiness.update(entidade);
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response listagem(String json, @Context HttpHeaders headers){
		...
		Cidade example = gson.fromJson(json, Cidade.class);
		List<Cidade> list = cidadeBusiness.findByExample(example);

		byte[] bytesReport = null;
		try {
			bytesReport = cidadeBusiness.listagemPDF(this.httpServletResquest, usuario, list);
		...
		return Response.ok(bytesReport).header("Access-Control-Allow-Origin", frontEndResponse)
				...
				.header("Content-Disposition", "filename=\"" + "cidadeList" + ".pdf\"").build();
	}
