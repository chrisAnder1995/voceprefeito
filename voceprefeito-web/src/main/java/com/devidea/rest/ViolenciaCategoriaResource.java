...
@Path("/violenciaCategoria")
@Produces(MediaType.APPLICATION_JSON)
@Stateless
public class ViolenciaCategoriaResource {
...
// VARIABLES_RESOURCE
	...
	public ViolenciaCategoriaResource() {
	...
	public Response getById(Integer id, @Context HttpHeaders headers){
		...
		ViolenciaCategoria entidade = violenciaCategoriaBusiness.findById(id);
	...
	public Response getAll(@Context HttpHeaders headers){
		...
		List<ViolenciaCategoria> list = violenciaCategoriaBusiness.findAll();
	...
	public Response getAllFast(@Context HttpHeaders headers){
		...
		List<ViolenciaCategoria> list = violenciaCategoriaBusiness.findAllFast();
	...
	public Response getAll(Integer page, @Context HttpHeaders headers){
		...
		List<ViolenciaCategoria> list = violenciaCategoriaBusiness.findAll(page);
	...
	public Response getByExample(String json, @Context HttpHeaders headers){
		...
		ViolenciaCategoria example = gson.fromJson(json, ViolenciaCategoria.class);
		List<ViolenciaCategoria> list = violenciaCategoriaBusiness.findByExample(example,example.getListPage(),example.getListPageSize());
	...
	public Response create(String json, @Context HttpHeaders headers){	
		...
		ViolenciaCategoria entidade = gson.fromJson(json, ViolenciaCategoria.class);
		entidade.setDataCadastro(new Date());
		entidade.setUsuarioCadastro(usuario);
		
		try {
			entidade = violenciaCategoriaBusiness.create(entidade);			
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response update(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = violenciaCategoriaBusiness.update(entidade);
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response listagem(String json, @Context HttpHeaders headers){
		...
		ViolenciaCategoria example = gson.fromJson(json, ViolenciaCategoria.class);
		List<ViolenciaCategoria> list = violenciaCategoriaBusiness.findByExample(example);

		byte[] bytesReport = null;
		try {
			bytesReport = violenciaCategoriaBusiness.listagemPDF(this.httpServletResquest, usuario, list);
		...
		return Response.ok(bytesReport).header("Access-Control-Allow-Origin", frontEndResponse)
				...
				.header("Content-Disposition", "filename=\"" + "violenciaCategoriaList" + ".pdf\"").build();
	}
