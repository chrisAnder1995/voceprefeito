...
public class PoliticaResource {
	...
	@EJB
	private PaisBusiness politicaBusiness;
...
// VARIABLES_RESOURCE
	...
	public Response getById(Integer id, @Context HttpHeaders headers){
		...
		Politica entidade = politicaBusiness.findById(id);
	...
	public Response getAll(@Context HttpHeaders headers){
		...
		List<Politica> list = politicaBusiness.findAll();
	...
	public Response getAllFast(@Context HttpHeaders headers){
		List<Politica> list = politicaBusiness.findAllFast();
	...
	public Response getAll(Integer page, @Context HttpHeaders headers){
		...
		List<Politica> list = politicaBusiness.findAll(page);
	...
	public Response getByExample(String json, @Context HttpHeaders headers){
		...
		List<Politica> list = politicaBusiness.findByExample(example,example.getListPage(),example.getListPageSize());
	...
	public Response create(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = politicaBusiness.create(entidade);			
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response update(String json, @Context HttpHeaders headers){	
		...
		try {
			entidade = politicaBusiness.update(entidade);
		} catch (Exception e) {			
			e.printStackTrace();
		}
	...
	public Response listagem(String json, @Context HttpHeaders headers){
		...
		Politica example = gson.fromJson(json, Politica.class);
		List<Politica> list = politicaBusiness.findByExample(example);

		byte[] bytesReport = null;
		try {
			bytesReport = politicaBusiness.listagemPDF(this.httpServletResquest, usuario, list);
		...
		return Response.ok(bytesReport).header("Access-Control-Allow-Origin", frontEndResponse)
				...
				.header("Content-Disposition", "filename=\"" + "politicaList" + ".pdf\"").build();
	}
